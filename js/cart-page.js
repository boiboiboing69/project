...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderCartPage() {
        ...
        if (cartItems.length === 0) {
            ...
            // cartPageSummaryDiv remains hidden
            ...
            if(cartItemsList) cartItemsList.style.display = 'block'; // Show list once populated
        ...
        cartItems.forEach((item) => {
            ...
            itemElement.innerHTML = `
                <img src="${item.image || 'https://via.placeholder.com/100x100'}" alt="${item.name}" class="cart-item-image"> {/* Dynamic Alt Text already here from previous logic */}
                ...
                <div class="cart-item-quantity">
                    <label for="qty-${itemKey.replace(/\W/g, '_')}">Qty:</label>
                    <input type="number" id="qty-${itemKey.replace(/\W/g, '_')}" class="quantity-input" value="${item.quantity}" min="1" data-item-key="${itemKey}" aria-label="Quantity for ${item.name}">
                </div>
                <div class="cart-item-subtotal">
                    <p>Item Total: <span class="item-subtotal-value">${formatCurrency(item.price * item.quantity)}</span></p>
                </div>
                <button class="btn-remove-item" data-item-key="${itemKey}" aria-label="Remove ${item.name} from cart">&times; Remove</button> {/* Enhanced ARIA Label */}
            `;
